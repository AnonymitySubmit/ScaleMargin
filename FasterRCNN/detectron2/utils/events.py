...
class CommonMetricPrinter(EventWriter):
    ...
    def write(self):
        ...
        self.logger.info(str.format(" {eta}iter: {iter}  {losses}  {non_losses}  {avg_time}{last_time}" + "{avg_data_time}{last_data_time} lr: {lr}  {memory}",
                                    eta=f"eta: {eta_string}  " if eta_string else "",
                                    iter=iteration,
                                    losses="  ".join(["{}: {:.4g}".format(k, v.median(storage.count_samples(k, self._window_size)))
                                                      for k, v in storage.histories().items() if "loss" in k]),
                                    non_losses="  ".join(["{}: {:.4g}".format(k, v.median(storage.count_samples(k, self._window_size)))
...
class EventStorage:
    ...
    def put_histogram(self, hist_name, hist_tensor, bins=1000):
        ...
        hist_params = dict(tag=hist_name, min=ht_min, max=ht_max, num=len(hist_tensor), sum=float(hist_tensor.sum()), sum_squares=float(torch.sum(hist_tensor**2)),
                           bucket_limits=hist_edges[1:].tolist(), bucket_counts=hist_counts.tolist(), global_step=self._iter)
