...
class IterationTimer(HookBase):
    ...
    def after_train(self):
        ...
        if num_iter > 0 and total_time_minus_hooks > 0:
            # Speed is meaningful only after warmup NOTE this format is parsed by grep in some scripts
        ...
        logger.info("Total training time: {} ({} on hooks)".format(str(datetime.timedelta(seconds=int(total_time))), str(datetime.timedelta(seconds=int(hook_time)))))
...
class PeriodicWriter(HookBase):
    ...
    def after_train(self):
        for writer in self._writers:
            # If any new data is found (e.g. produced by other after_train), write them before closing
            writer.write()
            writer.close()
...
class BestCheckpointer(HookBase):
    ...
    def __init__(self, eval_period: int, checkpointer: Checkpointer, val_metric: str, mode: str = "max", file_prefix: str = "model_best") -> None:
        """
        Args:
    ...
    def _best_checking(self):
        ...
        if self.best_metric is None:
            ...
            additional_state = {"iteration": metric_iter}
            self._checkpointer.save(f"{self._file_prefix}", **additional_state)
            self._logger.info(f"Saved best model as latest eval score for {self._val_metric} is "
                              f"{latest_metric:0.5f}, better than last best score "
                              f"{self.best_metric:0.5f} @ iteration {self.best_iter}.")
            self._update_best(latest_metric, metric_iter)
        else:
            self._logger.info(f"Not saving as latest eval score for {self._val_metric} is {latest_metric:0.5f}, "
...
class LRScheduler(HookBase):
    ...
    def before_train(self):
        ...
        if isinstance(self.scheduler, ParamScheduler):
            self._scheduler = LRMultiplier(self._optimizer, self.scheduler, self.trainer.max_iter, last_iter=self.trainer.iter - 1)
        ...
        if self._enable_predicate(self.trainer):
            if self._save_tensorboard:
                on_trace_ready = torch.profiler.tensorboard_trace_handler(
                    os.path.join(self._output_dir, "log", "profiler-tensorboard-iter{}".format(self.trainer.iter)), f"worker{comm.get_rank()}")
            else:
                on_trace_ready = None
            self._profiler = torch.profiler.profile(activities=self._activities, on_trace_ready=on_trace_ready, record_shapes=True,
        ...
        if (self.trainer.iter + 1) % self._period == 0 or (self.trainer.iter == self.trainer.max_iter - 1):
            if torch.cuda.is_available():
                ...
                self._logger.info((" iter: {} "
                                   ...
                                   " allocated_mem: {:.0f}MB ").format(self.trainer.iter, max_reserved_mb, reserved_mb, max_allocated_mb, allocated_mb))
