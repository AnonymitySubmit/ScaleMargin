...
class DatasetMapperTTA:
    ...
    def from_config(cls, cfg):
        return {"min_sizes": cfg.TEST.AUG.MIN_SIZES,
...
class GeneralizedRCNNWithTTA(nn.Module):
    ...
    def _batch_inference(self, batched_inputs, detected_instances=None):
        ...
        for idx, input, instance in zip(count(), batched_inputs, detected_instances):
            ...
            if len(inputs) == self.batch_size or idx == len(batched_inputs) - 1:
                outputs.extend(
                    self.model.inference(inputs, instances if instances[0] is not None else None, do_postprocess=False))
                inputs, instances = [], []
        return outputs
    ...
    def _merge_detections(self, all_boxes, all_scores, all_classes, shape_hw):
        ...
        merged_instances, _ = fast_rcnn_inference_single_image(all_boxes, all_scores_2d, shape_hw, 1e-8, 
