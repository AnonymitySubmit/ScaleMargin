...
class FastRCNNOutputLayers(nn.Module):
    ...
    def losses(self, predictions, proposals, pred_list=None, gt_box=None, margin_list=None, margin_max=None):
        """
        Args:
        ...
        if self.use_sigmoid_ce:
            ...
            if scale_margin:
                scores = adding_margin_discrete(scores, gt_classes, pred_list, gt_box, [1, 0.5, 0.25])
            elif magn_dynamic:
                margin_list, margin_obj, margin_max, size_batch = margin_dynamic_compute(scores, gt_classes, pred_list, gt_box, margin_list, margin_max)
                scores = margin_dynamic_adding(scores, gt_classes, pred_list, size_batch, margin_list, margin_obj, margin_max, init_margin=[0.75, 0.75, 0.75])

            # gt_classes中的目标索引从0开始一直到80, 81即背景
            # scores.shape=[512*bs, cls_num+1], gt_boxes.shape=[512*bs, 4]
            loss_cls = cross_entropy(scores, gt_classes, reduction="mean")

        losses = {"loss_cls": loss_cls, "loss_box_reg": self.box_reg_loss(proposal_boxes, gt_boxes, proposal_deltas, gt_classes)}
        return {k: v * self.loss_weight.get(k, 1.0) for k, v in losses.items()}, margin_list, margin_max
...
def adding_margin_discrete(logits, gt_class, pred_list, gt_box, init_margin_list):
    # pred_list长度为批量
    for i in range(len(pred_list)):
        # pred_list[i]为每张图片拥有的目标
        for j in range(len(pred_list[i])):

            area = (gt_box[i][j][2]-gt_box[i][j][0])*\
                   (gt_box[i][j][3]-gt_box[i][j][1])
            ...
            # 经过测试发现如果将单个目标的最大Pred赋予其他Pred会导致训练在大约4.6k处发散

            # torch.set_printoptions(precision=2,sci_mode=False)

            # 经过测试发现每个GT的所有Pred在训练末期的正确类logit均能达到9~12, 均是有效Pred
            # 但还是额外设置判断, 当正确类logit小于-1时跳过该Pred, 因为初始logit∈[-1, 0]
            for k in pred_list[i][j]:
                # -----------------------test code-----------------------
                # if init_margin == 1.0:
                #     print(f"small margin: {logits[k][int(gt_class[k])].item()}")
                # if init_margin == 0.5:
                #     print(f"midle margin: {logits[k][int(gt_class[k])].item()}")
                # if init_margin == 0.25:
                #     print(f"large margin: {logits[k][int(gt_class[k])].item()}")
                # -----------------------test code-----------------------
                if logits[k][int(gt_class[k])].item() < -1: continue
                logits[k][int(gt_class[k])] -= init_margin
            
            # print("\n")
    
    return logits

def margin_dynamic_compute(logits, gt_class, pred_list, gt_box, margin_list, margin_max):
    # initialize some parameters
    obj_len, size_batch = [0, 0, 0], copy.deepcopy(pred_list)
    ...
    for i in range(len(pred_list)):
        ...
        for j in range(len(pred_list[i])):
            ...
            if 0 < area <= 32**2:
                size_batch[i][j] = 'small'
                obj_len[0] += 1
            elif 32**2 < area <= 96**2:
                size_batch[i][j] = 'midle'
                obj_len[1] += 1
            elif 96**2 < area:
                size_batch[i][j] = 'large'
                obj_len[2] += 1
    ...
    if obj_len[0] >= save_num: margin_list[0] = []
    if obj_len[1] >= save_num: margin_list[1] = []
    if obj_len[2] >= save_num: margin_list[2] = []
    ...
    for i in range(len(pred_list)):
        ...
        for j in range(len(pred_list[i])):
            ...
            # 每个目标有多个Pred框
            for k in pred_list[i][j]:
                margin_pred.append(logits[k][int(gt_class[k])].item())
            
            # if order == 0:
            #     print(f"small margin: {len(pred_list[i][j])}")
            #     print(max(margin_pred))
            # if order == 1:
            #     print(f"midle margin: {len(pred_list[i][j])}")
            #     print(max(margin_pred))
            # if order == 2:
            #     print(f"large margin: {len(pred_list[i][j])}")
            #     print(max(margin_pred))
            
            # print("\n")

            # 当GT没有Pred框时跳过
            if len(margin_pred) == 0:
            ...
            # 排除负样本, 负样本logit值会不断减小, 而初始值∈[-1, 1]
            # 有的样本初始小于0但最终远大于0, 有的样本边距全程小于0
    ...
    if obj_len[0] < save_num: margin_list[0] = margin_list[0][-save_num:]
    if obj_len[1] < save_num: margin_list[1] = margin_list[1][-save_num:]
    if obj_len[2] < save_num: margin_list[2] = margin_list[2][-save_num:]

    return margin_list, margin_obj, margin_max, size_batch

def margin_dynamic_adding(logits, gt_class, pred_list, size_batch, margin_list, mgn_obj, max_m, init_margin):
    # initialize some parameters
    ...
    for i in range(len(pred_list)):
        ...
        for j in range(len(pred_list[i])):
            ...
            if size_batch[i][j] == 'small':
                init_mgn, order, counter = init_margin[0], 0, counter_s
                counter_s += 1
            elif size_batch[i][j] == 'midle':
                init_mgn, order, counter = init_margin[1], 1, counter_m
                counter_m += 1
            elif size_batch[i][j] == 'large': 
                init_mgn, order, counter = init_margin[2], 2, counter_l
                counter_l += 1
            ...
            if mgn_obj[order][counter] != 'occupy':

                # test code
                # if mgn_obj[order][counter] < 4:
                #     print(f"{size_batch[i][j]} max margin_pred: {round(mgn_obj[order][counter], 2)}")

                # 获取每个样本的边距偏移量, 并以此计算每个样本最终需要减去的边距值, 
                # 其中init_mgn锚定均值, 为了防止某个样本的值起飞, 必须使用max和min
                delta_margin = (margin_mean[order]-mgn_obj[order][counter]) / max_m[order]
                margin = init_mgn if delta_margin < 0 else (1 + delta_margin) * init_mgn

                # test code
                # print(f"{size_batch[i][j]}: mean: {round(margin_mean[order], 2)}, delta_margin: {round(delta_margin, 2)}, margin: {round(margin, 2)}")
                
                # ------------------------old version------------------------

                # 将单个Pred的logit值赋予其他Pred会导致训练发散
                for k in pred_list[i][j]:
                    if logits[k][int(gt_class[k])].item() <= 0: continue
                    logits[k][int(gt_class[k])] -= margin
                
                # ------------------------old version------------------------

            # else:

                # 给负样本进行最大logit赋给其他Pred操作, 降低负样本损失值, 但可能导致训练发散
                # 训练初期很多有效GT由于边距小于0被置为"occupy", 再进行此类操作会降低训练效率
                # if len(pred_list[i][j]) != 0: 
                #     margin_max = max([logits[k][int(gt_class[k])].item() for k in pred_list[i][j]])
                #     for k in pred_list[i][j]: logits[k][int(gt_class[k])] = margin_max + init_mgn
