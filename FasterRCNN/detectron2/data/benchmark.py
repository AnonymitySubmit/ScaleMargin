...
class DataLoaderBenchmark:
    ...
    def __init__(self, dataset, *, mapper, sampler=None, total_batch_size, num_workers=0, max_time_seconds: int = 90):
        """
        Args:
    ...
    def _log_time(self, msg, avg, all_times, distributed=False):
        ...
        if not distributed:
            logger.info(f"{msg}: avg={1.0/avg:.1f} it/s, "
        ...
        for idx, avg, percentiles in zip(count(), avg_per_gpu, percentiles_per_gpu):
            logger.info(f"GPU{idx} {msg}: avg={1.0/avg:.1f} it/s, "
                        f"p1={percentiles[0]:.2g}s, p5={percentiles[1]:.2g}s, "
                        f"p95={percentiles[2]:.2g}s, p99={percentiles[3]:.2g}s.")
    ...
    def benchmark_IPC(self, num_iter, warmup=10):
        ...
        loader = build_batch_data_loader(dataset, self.sampler, self.total_batch_size, num_workers=n)
        self._benchmark(iter(loader), num_iter * max(n, 1), warmup * max(n, 1), f"DataLoader ({n} workers, bs={self.per_gpu_batch_size}) w/o comm")
    ...
    def benchmark_distributed(self, num_iter, warmup=10):
        ...
        del loader
        self._log_time(f"DataLoader ({gpu} GPUs x {n} workers, total bs={self.total_batch_size})", avg, all_times, True)
