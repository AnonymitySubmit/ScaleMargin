...
from detectron2.structures import (BitMasks, Boxes, BoxMode, Instances, Keypoints, 
                                   PolygonMasks, RotatedBoxes, polygons_to_bitmask)
from detectron2.utils.file_io import PathManager

from . import transforms as T
from .catalog import MetadataCatalog

__all__ = ["SizeMismatchError",
...
def check_image_size(dataset_dict, image):
    ...
    if "width" in dataset_dict or "height" in dataset_dict:
        ...
        if not image_wh == expected_wh:
            raise SizeMismatchError("Mismatched image shape{}, got {}, expect {}.".format(" for image " + dataset_dict["file_name"] if "file_name" in dataset_dict else "",
                                                                                          image_wh, expected_wh,) + " Please check the width/height in your annotation.")
...
def transform_proposals(dataset_dict, image_shape, transforms, *, proposal_topk, min_box_size=0):
    ...
    if "proposal_boxes" in dataset_dict:
        ...
        boxes = transforms.apply_box(BoxMode.convert(dataset_dict.pop("proposal_boxes"), dataset_dict.pop("proposal_bbox_mode"), BoxMode.XYXY_ABS,))
        boxes = Boxes(boxes)
...
def transform_instance_annotations(annotation, transforms, image_size, *, keypoint_hflip_indices=None):
    ...
    if "segmentation" in annotation:
        ...
        if isinstance(segm, list):
            ...
            raise ValueError("Cannot transform segmentation of type '{}'!"
...
def build_augmentation(cfg, is_train):
    ...
    if is_train and cfg.INPUT.RANDOM_FLIP != "none":
        augmentation.append(T.RandomFlip(horizontal=cfg.INPUT.RANDOM_FLIP == "horizontal", vertical=cfg.INPUT.RANDOM_FLIP == "vertical",))
    return augmentation
