...
class Trainer:
    ...
    def train(self):
        ...
        # self.iter_normal, self.iter_stitcher, self.iter_datapool = 0, 0, 0

        # 初始化尺度损失值参数
        # self.loss_sum_small, self.loss_sum_medium, self.loss_sum_large = 0.0, 0.0, 0.0

        # 初始化梯度相似度参数
        # self.grad_last, self.cos_sim = 0.0, 0.0

        # 初始化尺度向ECE参数
        # detal_ece_small, accumu_scale_ece = [], [0.0, 0.0, 0.0, 0.0]

        # 初始化尺度损失值参数
        # loss_small_total, loss_midle_total, loss_large_total = {}, {}, {}
        ...
        # 初始化批次损失值参数
        # loss_curve = {i*8**2:[] for i in range(int(512**2 / 8**2)+1)}

        # 初始化尺度边距参数
        # margin_small, margin_midle = [], []
        # margin_large, margin_total = [], []
        # margin_list = [[] for i in range(int(512/32+1))]
        ...
        for self.epoch in range(self.start_epoch, self.max_epoch):
            ...
            for self.iter in range(self.max_iter): # self.max_iter=len(self.train_loader),即数据集被划分成多少个批次
                ...
                inps, targets = inps.to(self.data_type), targets.to(self.data_type)
                inps, targets = inps.to(self.device), targets.to(self.device)
                ...
                # 统计该批次的GT数量
                # def count_gtnum(targets):
                #     counter = 0
                #     # 总共有bs张图片
                #     for i in range(len(targets)):
                #         # 每张图片有120个GT框, 其中有大量填充值
                #         for j in range(len(targets[i])):
                #             # 排除填充值即行和为0
                #             if targets[i][j].sum() > 0:
                #                 counter += 1
                #     print(f"GT Num: {counter}")
                ...
                # self.ratio_scale = outputs["ratio_scale"]
                # del outputs["ratio_scale"]

                cls_list = outputs["cls_list"]
                cls_max  = outputs["cls_max"]
                cls_min  = outputs["cls_min"]
                del outputs["cls_list"]
                del outputs["cls_max"]
                del outputs["cls_min"]

                obj_list = outputs["obj_list"]
                obj_max  = outputs["obj_max"]
                obj_min  = outputs["obj_min"]
                del outputs["obj_list"]
                del outputs["obj_max"]
                del outputs["obj_min"]
                ...
                # tensorboard login all sorts of loss difference
                # loss_small_total, loss_midle_total, loss_large_total, loss_small_iou, loss_midle_iou, loss_large_iou, loss_small_cls, loss_midle_cls, loss_large_cls, \
                # loss_small_obj, loss_midle_obj, loss_large_obj = self.loss_difference(loss, loss_small_total, loss_midle_total, loss_large_total, \
                # loss_small_iou, loss_midle_iou, loss_large_iou, loss_iou, loss_small_cls, loss_midle_cls, loss_large_cls, loss_cls, loss_small_obj, loss_midle_obj, loss_large_obj, loss_obj)

                # tensorboard login whole network grad similarity
                # 由于每次抽样的样本都是不同的因此每次的梯度都几乎呈现90°
                # 如何剔除类别的影响展示出尺度对梯度的影响是个非常难的问题
                # self.layer_by_layer_grad_sim() # self.whole_network_grad_sim()

                # tensorboard login all scale margin in training
                # self.batch_scale_margin(margin)

                # divide loss value into bins to form loss curve
                # loss_curve = self.loss_distributer(loss_curve, loss_scale)
                
                # after_iter log needed information
                if (self.iter + 1) % self.exp.print_interval == 0:

                    # detal_ece_small.append(scale_ece[3]-scale_ece[1])
                    # for i in range(len(accumu_scale_ece)):
                    #     accumu_scale_ece[i] += scale_ece[i]
                    ...
                    if self.rank == 0:
                        ...
                        # if self.args.logger == 'tensorboard':
                            # self.tblogger.add_scalar("train/total_ece", scale_ece[0], (self.epoch) * self.max_iter + self.iter)
                            # self.tblogger.add_scalar("train/small_ece", scale_ece[1], (self.epoch) * self.max_iter + self.iter)
                            # self.tblogger.add_scalar("train/midle_ece", scale_ece[2], (self.epoch) * self.max_iter + self.iter)
                            # self.tblogger.add_scalar("train/large_ece", scale_ece[3], (self.epoch) * self.max_iter + self.iter)

                    self.meter.clear_meters()

                    # detal_ece_small = []
                    # accumu_scale_ece = [0.0, 0.0, 0.0, 0.0]
                # else:
                    # detal_ece_small.append(scale_ece[3]-scale_ece[1])
                    # for i in range(len(accumu_scale_ece)):
                    #     accumu_scale_ece[i] += scale_ece[i]
            ...
            # validation every epoch when searching optimal initial ratio
            # self.ratio_searching_evaluate()
            # assert self.epoch + 1 <= 5, "searching reaching end"
    ...
    def warmup_controller(self): 
        ...
        if self.epoch <= -1:
            ...
            if self.batch_flag != 4:
                if self.ratio_scale[0] < 0.1: # 判断小目标损失值, 当小目标损失值小于阈值时启动目标池增强
                    self.prefetcher = self.prefetcher_datapool
                    self.iter_datapool = self.iter_datapool + 1
    ...
    def batch_scale_margin(self, margin):
        # 每个batch都将检测头计算得到的margin列表拆解出小中大总四个部分进行记录
        margin_small_avg, margin_midle_avg, margin_large_avg, margin_total_avg = margin

        # 每轮都向tensorboard更新一次小中大总四个尺度的margin
        if self.rank == 0 and self.args.logger == 'tensorboard':
            self.tblogger.add_scalar("train/batch_small_margin", margin_small_avg, (self.epoch) * self.max_iter + self.iter)
            self.tblogger.add_scalar("train/batch_midle_margin", margin_midle_avg, (self.epoch) * self.max_iter + self.iter)
            self.tblogger.add_scalar("train/batch_large_margin", margin_large_avg, (self.epoch) * self.max_iter + self.iter)
            self.tblogger.add_scalar("train/batch_total_margin", margin_total_avg, (self.epoch) * self.max_iter + self.iter)
...
def make_parser():
    ...
    # pre-train params
    resum, prept = False, None
    # resum, prept = True, "./YOLOX_outputs/epoch_210_ckpt.pth"

    # pre-train setting
    parser.add_argument("--resume",              default=resum,  action="store_true",                help="resume training")
    parser.add_argument("-c", "--ckpt",          default=prept,  type=str,                           help="checkpoint file")
