...
class Trainer:
    ...
    def train(self):
        ...
        self.batch_flag, is_regular, temp_iter = 0, True, 0
        ...
        for self.epoch in range(self.start_epoch, self.max_epoch):
            ...
            for self.iter in range(self.max_iter): # self.max_iter=len(self.train_loader),即数据集被划分成多少个批次
                ...
                loss, margin = outputs["total_loss"], outputs["margin"]
                ratio_scale, loss_iou = outputs["ratio_scale"], outputs["loss_iou"]
                loss_cls, loss_obj = outputs["loss_cls"], outputs["loss_obj"]
                ...
                del outputs["ratio_scale"], outputs["loss_iou"], outputs["loss_cls"], outputs["loss_obj"], outputs["margin"], outputs["loss_scale"]

                # print(f"the scaled deltal loss: {self.scaler.scale(loss_iou[2] - loss_iou[0])}")
                ...
                # loss_small_iou, loss_midle_iou, loss_large_iou, \
                # loss_small_cls, loss_midle_cls, loss_large_cls, \
                # loss_small_obj, loss_midle_obj, loss_large_obj = self.loss_difference(\
                # loss, loss_small_total, loss_midle_total, loss_large_total, \
                ...
                # 训练中的多批次平滑margin, 用于训练时记录margin信息
                # margin_small, margin_midle, margin_large, margin_total = self.batch_scale_margin(margin, margin_small, margin_midle, margin_large, margin_total)

                # 记录每个batch的各尺度normalized margin
                # 证明不同尺度在相同时刻拥有相同的边界复杂度
                # margin_small, margin_midle, margin_large, margin_total = self.normalize_margin(targets, margin, margin_small, margin_midle, margin_large, margin_total)

                # divide loss value into bins to form loss curve
                # loss_curve = self.loss_distributer(loss_curve, loss_scale)

                # 记录每个batch的各尺度daptive margin
                for i in range(len(margin_list)):
                    margin_list[i].append(margin[4][i])

                # normalized margin的停止机制
                if len(margin_list[2]) == self.exp.print_interval * 20:
                    for i in range(len(margin_list)):
                        margin_list[i] = sum(margin_list[i]) / len(margin_list[i])
...
def yoloxdstre(ckpt_path):
    ...
    if world_size > 1: # 有卡就并行训练
        ...
        margin_list = main(exp, args) # 没卡就用CPU
